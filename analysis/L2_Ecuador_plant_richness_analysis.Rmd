...
# convert to sf
Ecuador_plants_sf <- st_as_sf(Ecuador_plants, coords = c("Latitude","Longitude"))
Ecuador_plants_sf <- st_set_crs(Ecuador_plants_sf, 4326) 
head(Ecuador_plants_sf)
```
...
spPointsPlot <-
  ...
  geom_sf(data = neighbors, color = "white") +
  geom_sf(data = ECpoly) +
  geom_sf(data = Ecuador_plants_sf, aes(fill = Scientific_name),pch = 21) +
